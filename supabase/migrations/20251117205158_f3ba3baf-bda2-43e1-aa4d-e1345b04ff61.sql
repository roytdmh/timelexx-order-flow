-- Harden RPC functions using RETURN QUERY and ensure single-row result

-- Drop existing functions to change body safely
DROP FUNCTION IF EXISTS public.rider_report_delivery(uuid, text);
DROP FUNCTION IF EXISTS public.admin_confirm_delivery(uuid);

-- Recreate rider_report_delivery
...
-- Recreate admin_confirm_delivery
...
BEGIN
  ...
  -- If no row was updated, raise a clear error
  IF NOT FOUND THEN
