-- Fix error in log_order_updates(): using set-returning jsonb_object_keys in IF caused "query returned more than one row" when multiple fields changed.
-- Replace with jsonb_object_length(changed_fields) > 0 check.
...
BEGIN
  ...
  IF jsonb_object_length(changed_fields) > 0 THEN
